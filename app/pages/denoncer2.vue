<template>
  <div
    ...
    style="background-image: linear-gradient(rgba(0,0,0,.6), rgba(0,0,0,.6)), url('/amongThem.png');"
  >
    <button
      @click="$router.back()"
    ...
    <h1 class="text-2xl mb-12 text-center drop-shadow-lg font-black" style="-webkit-text-stroke: 1px black;">
      Identifier l’imposteur parmi l’équipage
    </h1>
...
// Liste des 6 suspects
const suspects = [
  { id: 1, name: 'Mathis', image: '/mathis.png' },
  { id: 2, name: 'Laetitia', image: '/laeti.png' },
  { id: 3, name: 'Ahu', image: '/ahu.png' },
  { id: 4, name: 'Esteban', image: '/esteban.png' },
  { id: 5, name: 'Robin', image: '/robin.png' },
  { id: 6, name: 'Geoffroy', image: '/geoffroy.png' },
]

const selectedSuspect = ref<any>(null)
const culpritId = 2 // imposteur
...
function confirmAccusation() {
  ...
  if (id === culpritId) {
    router.push({ path: '/victoire2', query: { name, image } })
  } else {
    router.push({ path: '/defaite2', query: { name, image } })
  }
}
</script>
...
[data-avatar]:nth-child(5) { animation-delay: 0.4s; }
[data-avatar]:nth-child(6) { animation-delay: 0.5s; }
